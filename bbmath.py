...
def get_dimensionless(oil, gas, diameter, sigma, roho):
    '''
        Function to return lambdal, nfr and nvl
        Takes in oil/gas rate, diameter, sigma and oil density 
    '''
    return (oil / (oil + gas), (oil + gas) ** 2 / (G * diameter), oil * (roho / (G * sigma) ** 1 / 4))


def get_mix_properties(lambdal, roho, rohg, muo, mug):
    '''
...
def get_liquid_holdup(lambdal, nfr, nvl, theta, flop):
    """
    ...
    tran = False
    if flop == "tranup":
        a,b,c,e,f,g,h = FP["segup"]
        a2,b2,c2,e2,f2,g2,h2 = FP["intup"]
        tran = True
    elif flop == "trando":
        a,b,c,e,f,g,h = FP["segdo"]
        a2,b2,c2,e2,f2,g2,h2 = FP["intdo"]
        tran = True
    else:
        a,b,c,e,f,g,h = FP[flop]
...
def get_reynold_num(lambdal, rohn, mun, diameter, mr):
    '''
...
def get_friction_factor(ren, diameter, epsilon):
    '''
...
def correct_friction_f(lambdal, hlo, ff):
    '''
...
def get_rohs(hlo, roho, rohg):
    ''' Just for rohs '''
    return roho * hlo + rohg * (1 - hlo)


def get_pressure_gradient(ff, rohn, mr, diameter, rohs, theta):
    ''' Just for pressure gradient '''
    return -(ff * rohn * mr ** 2) / (2 * diameter) - rohs * G * np.sin(theta)
